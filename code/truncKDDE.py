...
# -*- coding: utf-8 -*-
"""
Created on Thu May 11 11:06:28 2017

@author: veysiyildiz
"""
'''
Calculate the kernel values. All functions are implemented from J_Shaker_EfficientMultivariateKernelDensityDerivativeEstimation_JSPS_2014 paper
...
def sec_der_k_one_dim(x,kernel_type='Gaussian'):
    ...
    if kernel_type=='Gaussian':
        ...
        """
#        print 'sefc der one dim:' , x
        return -k_one_dim(x, kernel_type='Gaussian') + (x**2)*k_one_dim(x, kernel_type='Gaussian')
    else:
        print "I dont know how to calculate this Kernel. Please update k_one_dim(x, kernel_type='Gaussian') and required derivative functions"
        return 
...
def first_der_K_mulvar(x, kernel_type='Gaussian'):
    ...
    input = x vector(d,1 aarray)
    output: gradient at point x as d*1 vector 
    """
#    print ' k mulvar', x
    dim=x.size
...
def K_S(x,S, kernel_type='Gaussian'):
    ...
    return beta_S*K_mulvar_I_Sca(np.dot(S,x), kernel_type)
...
def first_der_K_S(x,S, kernel_type='Gaussian'):
    ...
    """ 
#    print 'first ks:' ,x
#    print 'first ks input :', np.dot(S,x)
    return np.linalg.det(S)* np.dot(first_der_K_mulvar(np.dot(S,x), kernel_type) , S)
...
def sec_der_K_S(x,S, kernel_type='Gaussian'):
    ...
    """  
#    print 'sec ks:' ,x
#    print 'sec ks input :', np.dot(S,x)
    return np.linalg.det(S) * np.dot (S.T , np.dot(sec_der_K_mulvar(np.dot(S,x), kernel_type) , S) )
...
def fx(w,S,X_i,x, kernel_type='Gaussian'):
    ...
    inputs
        w: A python list which consists of weights of each kernel(weights should be floats)
        S: A python list which consists of coveriance matrix of each kernel(d*d matrix)
        X_i: A python list which consists of locations of each kernel(locations should be vector arrays( d*1 array))
        x: point to calculate density value. (d*1 array)
...
def gx(w,S,X_i,x, kernel_type='Gaussian'):
    ...
    inputs
        ...
        X_i: A python list which consists of locations of each kernel(locations should be vector arrays( d*1 array))
        x: point to calculate density value. (d*1 array)
    output: gradient at point x (d*1 array)
    """
#    print 'gx:' ,x
...
def Hx(w,S,X_i,x, kernel_type='Gaussian'):
    ...
    inputs
        ...
        X_i: A python list which consists of locations of each kernel(locations should be vector arrays( d*1 array))
        x: point to calculate density value. (d*1 array)
    output: hessian at point x (d*d matrix)
...
def ln_gx(w,S,X_i,x, kernel_type='Gaussian'):
    ...
    inputs
        ...
        X_i: A python list which consists of locations of each kernel(locations should be vector arrays( d*1 array))
        x: point to calculate density value. (d*1 array)
    output: gradient log at point x (d*1 array)
...
def ln_Hx (w,S,X_i,x, kernel_type='Gaussian'):
    ...
    inputs
        ...
        X_i: A python list which consists of locations of each kernel(locations should be vector arrays( d*1 array))
        x: point to calculate density value. (d*1 array)
    output: hessian log at point x (d*d matrix)
